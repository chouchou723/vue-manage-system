...
<el-form :model="actSchool" id='actSchool1'>
  ...
  <div><div class='editSpan' @click='editReturn(item.index,item)' v-if="new Date().getTime()-new Date(item.create_at).getTime()<7200000 && item.tmk.uname == userName"></div></div>
  </el-col>
</el-row>
         </div>
...
<script>
var token,user
import { cityList,put_community,create_community,activateResource,campusList,getUserList,getUserCommList,getTMK,
  sourceList,repeatStudentList,getUserInviteList,put_student} from '../../api/api';
  ...
  export default {
    data() {
      ...
      return {
        ...
        schools:[],
        list:[],
        sourceId:3,//替换成vuex的getUId
        getStauts:0,//getStauts
        student:{
                  name:'',
                  sex:'',
                  age:'',
    ...
    methods: {
      ...
      querySearchAsync(queryString, cb) {//转介绍时 输入姓名 同步搜索
        let para={input:queryString}
        repeatStudentList(token,para).then(res=>{
          // console.log(res.data)
          if(res.data.length != 0){
              this.studentsList = res.data.map(item=>{
                return {referral_uid:item.label.uid,value:item.label.nickname,familys_name:item.label.familys_name,mobile:item.label.mobile}
              })
              var studentsList = this.studentsList;
              if(isNaN(queryString)){

              var results = studentsList.filter(item=>{return item.value.indexOf(queryString)>-1});
              }else{
      ...
      getRegion(){//获取城市，就近校区
        ...
        let para1 = {city_id:this.form.city_id}
        campusList(para1,token).then(res=>{
          this.schoolsNear = res.data.data
        })
      },
      ...
      editReturn(index,item){//点击修改沟通记录
        ...
        this.commuForm.remind_time = item.remind_time;
         this.commuForm.customer_id ='12' //this.getUserId;
      ...
      addU(){//点击修改用户资料
        ...
        this.form={
          ...
          parent1:data.famliy[1].uname,
          con:data.famliy[0].relation,
          con1:data.famliy[1].relation,
          phone:data.famliy[0].mobile,
          phone1: data.famliy[1].mobile,
          familys:'',
    ...
    created(){
       ...
       let para = {customer_id:'2451'}
      getUserList(token,para).then(res=>{//获取用户资料
        let{names,age,sex,source_name,school_name,created,cc_name} = res.data.info;
       this.student = {
          name: names,
          age: age,
          sex:sex,
